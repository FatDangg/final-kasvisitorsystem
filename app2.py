...
logging.info("? Logging initialized and working.")
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://kasvisitor:fLK9wt6pkAGCEbaJ@localhost/visitorsdb'
...
def save_photo(photo_base64, identifier):
    ...
    try:
        ...
        logging.error("Error saving photo:", e)
        return None
...
def generate_pdf(pdf_file_path, first_name, last_name, purpose, finding, visitor_photo_path, school_logo_path):
    ...
    try:
        ...
        logging.error("Error loading school logo:", e)
    ...
    try:
        ...
        logging.error("Error loading visitor photo:", e)

    c.showPage()
    c.save()

def auto_print_label(first_name, last_name, purpose, finding, photo_path):
    logging.info("?? Entered auto_print_label()")
    """
    ...
    try:
        ...
        logging.error("Error opening/resizing visitor photo:", e)
        photo = None
    ...
    logging.info("? settung up the printer...")
    ...
    try:
        logging.info("?? Converting label for QL-800...")
        ...
        logging.info("? Label instructions prepared.")
    except Exception as e:
        logging.error("? Error during label conversion", exc_info=True)
        return
    
    logging.info("? Sending label to printer...")
    send(
    ...
    logging.info("✅ Label (text at top, photo below) sent to QL-800.")

@app.route('/submit', methods=['POST'])
def submit_visitor():
    logging.info("? /submit endpoint triggered.")
    """
    ...
    try:
        ...
        logging.info("Error printing to Brother QL-800:", e)
